...
export function NodeSettings({
  ...
  const canStart = state?.reachability === "unavailable" && state.ownership === "none";
  ...
  return (
    ...
    >
      ...
      >
        <span className="status-label">{state?.reachability ?? "Checking"}</span>
      </TaskRow>
      ...
      {state?.ownership === "console" && (
        <p className="task-help">
          Closing the window keeps this node running. Quit from the tray stops it.
        </p>
      )}
      {state?.last_error && <p role="alert">{state.last_error}</p>}
      {error && !pending && <p role="alert">{error}</p>}
      <div className="node-actions">
        <button disabled={busy || !canStart} onClick={() => void controls.run("start")}>
          Start node
        </button>
        <button disabled={busy || !canControl} onClick={() => setPending("restart")}>
          Restart node
        </button>
        <button disabled={busy || !canControl} onClick={() => setPending("stop")}>
          Stop node
        </button>
      ...
      {pending && (
        ...
        >
          <p>
            Connected apps will lose their local connection while this node is stopped. Console
            checks ownership again before proceeding.
          </p>
