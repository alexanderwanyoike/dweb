...
it("opens Advanced directly on the daemon log and refreshes the real output", async () => {
  ...
  await act(async () => {});
  expect(
    screen.getByRole("region", { name: "Daemon log output" }),
  ).toHaveTextContent("node started");
  expect(screen.getByRole("link", { name: "Relays" })).toHaveAttribute(
    "href",
    "/relays",
  );
  ...
  expect(
    screen.getByRole("region", { name: "Daemon log output" }),
  ).toHaveTextContent("peer connected");
  cleanup();
