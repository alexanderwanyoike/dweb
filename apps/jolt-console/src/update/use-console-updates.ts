...
export function useConsoleUpdates(client: ConsoleUpdateClient, lifecycle: DaemonLifecycleClient) {
  ...
  const install = () =>
    action.run(async () => {
      if (!check?.available) throw new Error("Check for an available update before installing.");
      setInstalling(true);
      try {
