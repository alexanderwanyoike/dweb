...
it("shows actual logs and lets the user review exact support details", async () => {
  ...
  const view = render(<DiagnosticsPage snapshot={snapshot()} lifecycleClient={lifecycleClient} />);
  expect(await screen.findByText("A real test log line")).toBeVisible();
  expect(screen.getByText("12D3KooPeer")).toBeVisible();
  await userEvent.click(screen.getByRole("button", { name: "Prepare support details" }));
  expect(screen.getByRole("dialog")).toHaveTextContent("12D3KooPeer");
  expect(screen.getByRole("dialog")).toHaveTextContent("Nothing is sent automatically");
  view.rerender(
    <MemoryRouter>
      <DiagnosticsPage
  ...
  expect(screen.getByRole("dialog")).not.toHaveTextContent("changed-after-review");
